...
module.exports = {
    ...
    init: function (hostIp, ports) {
        ...
        var compiled = compileWithPrefix(template, params);

        return "\n" + compiled;
    },
    update: {
        prompt: function (latest, current) {
            var template = [];
            template.push("{red:----------------------------------------}");
            template.push("Update Available: {green:{:latest:}} {cyan:(current: {:current:})}");
            template.push("Run {yellow:npm update -g browser-sync} to update");
            template.push("{red:----------------------------------------}\n");

            var compiled = compileWithPrefix(template, {latest: latest, current: current});
            return "\n\n" + compiled;
        },
        progress: function () {
            var template = [];
            template.push("Checking NPM for updates...");
            template.push("(you can disable this in the options)");
            return compileWithPrefix(template);
        },
        confirm:function () {
            var template = [];
            template.push("We checked - you're running the latest version of BrowserSync {green::)}");
            template.push("(You can disable this check in the options)\n");
            return compileWithPrefix(template);
        }
